...
{
    Q_UNUSED(midiFilePath);
    Q_UNUSED(brrFilePath);
...
{
    // Убираем неиспользуемые переменные или используем Q_UNUSED
    int diff = sample - state.prev_sample;
    Q_UNUSED(diff); // Помечаем как неиспользуемую
    
    int step = step_table[state.prev_index];
    Q_UNUSED(step); // Помечаем как неиспользуемую
    
    // Квантование
    int nibble = 0;
    Q_UNUSED(nibble); // Помечаем как неиспользуемую
