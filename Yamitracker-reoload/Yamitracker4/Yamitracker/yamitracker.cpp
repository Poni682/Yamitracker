...
void Yamitracker::playTrack(int track)
{
    ...
    QByteArray audioData = wavWriter->getTrackAudioData(track, true);
    ...
    if (!info.isFormatSupported(format)) {
        ui->statusLabel->setText("Аудиоформат не поддерживается");
        return;
    }
...
void Yamitracker::playAllTracks()
{
    ...
    isPlayingBack = true;
    ui->statusLabel->setText("Воспроизведение всех дорожек...");
...
void Yamitracker::onNoteOffReceived(const QString &data)
{
    ...
    if (wavWriter->isRecording() && noteStartTimes.contains(data)) {
        ...
        if (ok) {
            ...
            wavWriter->addNote(currentTrack, noteStartTime, noteNumber, noteVelocity, duration);
