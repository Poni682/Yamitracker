...
void Yamitracker::initializeTrackControls()
{
    ...
    for (int i = 0; i < SimpleWavWriter::MAX_TRACKS; i++) {
        ...
        selectCheck->setObjectName(QString("trackSelectCheck_%1").arg(i)); // MAYBE ЭТОТ БЛОК КОДА ВСЕ ЛОМАЕТ ПО ПРИЧИНЕ ОДНОВРЕМЕНОГО ПРОИЗВЕДЕНИЯ, ТК У НАС ИДЕТ ПАРАЛЛЕЛЬНАЯ ЗАПИСЬ
        selectCheck->setChecked(true);
        connect(selectCheck, &QCheckBox::stateChanged, this, &Yamitracker::onTrackSelectChanged);
        trackSelectChecks.append(selectCheck);
...
{
    if (wavWriter->isRecording() && isGlobalRecording) {
        QMessageBox::warning(this, "Переключение дорожки",
            "Переключение дорожки во время записи может привести к рассинхронизации. "
            "Все дорожки используют общее время записи.");
    }
...
void Yamitracker::onNoteOnReceived(const QString &data, int velocity)
{
    ...
    if (isGlobalRecording) {
        double currentTime = QDateTime::currentMSecsSinceEpoch() / 1000.0 - globalRecordingStartTime;
        bool ok;
        int noteNumber = data.toInt(&ok, 16);
        if (ok) {
            // ТОЛЬКО сохраняем время начала, не добавляем ноту
            noteStartTimes[data] = currentTime;
            noteVelocities[data] = velocity;  // Сохраняем velocity
            noteInstruments[data] = instrumentComboBox->currentData().toInt();  // Сохраняем инструмент
            noteTracks[data] = currentTrack;  // Сохраняем текущую дорожку
        }
    }
}
...
void Yamitracker::onNoteOffReceived(const QString &data)
{
    ...
    if (isGlobalRecording && noteStartTimes.contains(data)) {
        double currentTime = QDateTime::currentMSecsSinceEpoch() / 1000.0 - globalRecordingStartTime;
        bool ok;
        int noteNumber = data.toInt(&ok, 16);
        if (ok) {
            ...
            int noteVelocity = noteVelocities.value(data, 100);
            int currentInstrument = noteInstruments.value(data, instrumentComboBox->currentData().toInt());
            int track = noteTracks.value(data, currentTrack);
            
            // Добавляем ноту ОДИН РАЗ с правильными параметрами
            wavWriter->addNote(track, noteStartTime, noteNumber, noteVelocity, duration, currentInstrument);
            
            noteStartTimes.remove(data);
            noteVelocities.remove(data);
            noteInstruments.remove(data);
            noteTracks.remove(data);
...
void Yamitracker::onPlayClicked()
{
    ...
    // Если это первая запись в сессии, начинаем глобальную запись
    if (!isGlobalRecording) {
        globalRecordingStartTime = QDateTime::currentMSecsSinceEpoch() / 1000.0;
        isGlobalRecording = true;
        qDebug() << "Начало глобальной записи. Все дорожки будут синхронизированы.";
    }
...
void Yamitracker::generateInstrumentMelody(int instrument)
{
    ...
    // Время относительно глобального начала записи
    double currentGlobalTime = QDateTime::currentMSecsSinceEpoch() / 1000.0 - globalRecordingStartTime;
    
    // Разные мелодии для разных инструментов
    switch (instrument) {
    case 1: // CrandPno (Гранд-пианино)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 60, 100 * volumeMultiplier, 1.0, instrument); // C4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.5, 64, 80 * volumeMultiplier, 0.5, instrument);  // E4
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.0, 67, 90 * volumeMultiplier, 1.0, instrument);  // G4
        wavWriter->addNote(currentTrack, currentGlobalTime + 2.0, 72, 100 * volumeMultiplier, 1.0, instrument); // C5
        break;
        
    case 2: // BritePno (Яркое пианино)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 60, 100 * volumeMultiplier, 0.3, instrument); // C4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.1, 64, 90 * volumeMultiplier, 0.3, instrument);  // E4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.2, 67, 80 * volumeMultiplier, 0.3, instrument);  // G4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.3, 72, 100 * volumeMultiplier, 0.5, instrument); // C5
        break;
        
    case 3: // Harpsi (Клавесин)
    case 6:
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 60, 100 * volumeMultiplier, 0.1, instrument); // C4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.1, 62, 90 * volumeMultiplier, 0.1, instrument);  // D4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.2, 64, 100 * volumeMultiplier, 0.1, instrument); // E4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.3, 65, 90 * volumeMultiplier, 0.1, instrument);  // F4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.4, 67, 100 * volumeMultiplier, 0.2, instrument); // G4
        break;
        
    case 28: // FngrBass (Фингер-бас)
    case 33:
    case 39:
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 36, 120 * volumeMultiplier, 1.0, instrument); // C2 (бас)
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.0, 40, 100 * volumeMultiplier, 0.5, instrument); // E2
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.5, 43, 110 * volumeMultiplier, 1.0, instrument); // G2
        wavWriter->addNote(currentTrack, currentGlobalTime + 2.5, 48, 100 * volumeMultiplier, 0.5, instrument); // C3
        break;
        
    case 40: // SlapBas (Слэп-бас)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 36, 150 * volumeMultiplier, 0.2, instrument); // C2 (щелчок)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.3, 40, 120 * volumeMultiplier, 0.3, instrument); // E2 (пул)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.7, 43, 140 * volumeMultiplier, 0.2, instrument); // G2 (щелчок)
        break;
        
    case 29: // Ovrdrive (Овердрайв-гитара)
    case 37:
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 40, 100 * volumeMultiplier, 1.5, instrument); // E2
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.5, 43, 90 * volumeMultiplier, 1.0, instrument);  // G2
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.0, 45, 100 * volumeMultiplier, 0.5, instrument); // A2
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.5, 47, 110 * volumeMultiplier, 1.0, instrument); // B2
        break;
        
    case 48: // String (Струнные)
    case 49: // SlowStr (Медленные струнные)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 60, 80 * volumeMultiplier, 2.0, instrument);  // C4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.5, 64, 70 * volumeMultiplier, 1.5, instrument);  // E4
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.0, 67, 75 * volumeMultiplier, 2.0, instrument);  // G4
        break;
        
    case 81: // LeadSx (Саксофон)
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 65, 90 * volumeMultiplier, 1.5, instrument);  // F4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.3, 67, 85 * volumeMultiplier, 0.5, instrument);  // G4
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.8, 69, 95 * volumeMultiplier, 2.0, instrument);  // A4
        break;
        
    case 95: // Bright (Яркий звук)
    case 96:
    case 100:
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 72, 100 * volumeMultiplier, 0.2, instrument); // C5
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.2, 76, 90 * volumeMultiplier, 0.2, instrument);  // E5
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.4, 79, 100 * volumeMultiplier, 0.2, instrument); // G5
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.6, 84, 110 * volumeMultiplier, 0.3, instrument); // C6
        break;
        
    default:
        // Стандартная мелодия для неизвестных инструментов
        wavWriter->addNote(currentTrack, currentGlobalTime + 0.0, 60, 100 * volumeMultiplier, 1.0, instrument); // C4
        wavWriter->addNote(currentTrack, currentGlobalTime + 1.0, 64, 80 * volumeMultiplier, 1.0, instrument);  // E4
        wavWriter->addNote(currentTrack, currentGlobalTime + 2.0, 67, 90 * volumeMultiplier, 1.0, instrument);  // G4
        break;
    }
}
