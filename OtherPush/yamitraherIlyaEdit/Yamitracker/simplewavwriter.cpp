...
bool SimpleWavWriter::saveToFile(const QString &filename)
{
    ...
    // Выводим информацию о каждой дорожке
    qDebug() << "Информация о дорожках";
    for (int i = 0; i < MAX_TRACKS; i++) {
        if (!tracks[i].events.isEmpty()) {
            qDebug() << "Дорожка" << i << ":" 
                     << "нот:" << tracks[i].events.size()
                     << "время начала:" << tracks[i].events.first().time
                     << "время конца:" << (tracks[i].events.last().time + tracks[i].events.last().duration);
        }
    }
...
// НОВЫЙ МЕТОД: Конвертирует тики в строку задержки
QString SimpleWavWriter::ticksToDelayString(int ticks) const
{
    ...
    if (ticks >= 2000) return QString("D_2");
    if (ticks >= 1000) return QString("D_1");
    if (ticks >= 500) return QString("D_D2");
    if (ticks >= 250) return QString("D_D4");
    if (ticks >= 125) return QString("D_D8");
    if (ticks >= 62) return QString("D_D16");
    if (ticks >= 31) return QString("D_2");
    if (ticks >= 16) return QString("D_4");
    if (ticks >= 8) return QString("D_8");
    if (ticks >= 4) return QString("D_16");
    if (ticks >= 2) return QString("D_32");
    
    return QString("D_32");
}

// НОВЫЙ МЕТОД: Создает паттерн из событий дорожки
QString SimpleWavWriter::generatePattern(int trackIndex) const
{
    ...
    for (const NoteEvent &event : sortedEvents) {
        ...
        if (delay > 0) {
            ...
            QString delayCmd = ticksToDelayString(delayTicks);
            if (!delayCmd.isEmpty()) {
...
// ИСПРАВЛЕННЫЙ МЕТОД: Сохранение в музыкальный файл Pently
bool SimpleWavWriter::saveToMusicFile(const QString &filename)
{
    ...
    if (totalTicks < 1) totalTicks = 100; // Минимальная длительность
    
    qDebug() << "Общая длительность:" << maxDuration << "секунд =" << totalTicks << "тиков";
    ...
    // Ждем полную длительность композиции
    ...
    qDebug() << "Максимальная длительность:" << maxDuration << "сек";
    qDebug() << "В тиках:" << totalTicks;
...
QByteArray SimpleWavWriter::mixTracks(const QVector<int> &trackIndices)
{
    ...
    QVector<qint32> mixBuffer(totalSamples, 0);  // Используем qint32 для предотвращения клиппинга
    ...
    for (int trackIndex : trackIndices) {
        ...
        for (const NoteEvent &event : track.events) {
            ...
            for (int i = 0; i < noteSamples && startSample + i < totalSamples; i++) {
                // Прибавляем ноту к миксу (используем qint32)
                qint32 mixed = mixBuffer[startSample + i] + noteData[i];
                mixBuffer[startSample + i] = mixed;
            }
        }
    }
    
    // Конвертируем буфер в QByteArray с ограничением (clamping)
    QByteArray result;
    ...
    for (int i = 0; i < totalSamples; i++) {
        ...
        qint32 sample = mixBuffer[i];
        if (sample > 32767) sample = 32767;
        if (sample < -32768) sample = -32768;
        resultData[i] = static_cast<qint16>(sample);
    }
    return result;
}
